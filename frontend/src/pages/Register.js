...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const res = await axios.post('https://ai-bug-tracker-omega.vercel.app/api/auth/register', form);
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={styles.field}>
            ...
            <input name="name" value={form.name} onChange={handleChange} required style={styles.input} placeholder="Sahil Deodhe" />
          </div>
          <div style={styles.field}>
        ...
        <p style={styles.link}>Already have an account? <Link to="/login" style={{ color: '#7c3aed' }}>Sign In</Link></p>
      </div>
    </div>
  );
}
